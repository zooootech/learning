...
# 初めにスタート地点の座標と操作回数（1 からスタート）をキューに入れ、隣接する移動可能なマスの座標と操作回数 + 1 をqueueに入れます。 例. 上下左右のマスに移動可能な場合、（移動可能なマスの座標を 1, 2, 3, 4 と表現する） queue の先頭 ← (1, 1 + 1), (2, 1 + 1), (3, 1 + 1), (4, 1 + 1) → queue の末尾
