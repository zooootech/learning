...
# 各要素を引数として順にブロックを実行し、ブロックが真を返した最初の要素の位置を返します。一つも真にならなかった場合は nil を返します。
...
for y in 0..(h - 1)
  for x in 0..(w - 1)
    if array[y][x] == "A"
      a_paint << [y, x, a_distance]
    end
    if array[y][x] == "B"
      b_paint << [y, x, b_distance]
    end
  end
end
...
if ab == "B"
  ...
  aadd += 1  # 後の処理で、 A のキューに要素として追加する配列の添字が 2 の要素。距離を表す。先攻が B の場合、 A の最初のターンでの距離を 1 にしておくのは、後でターン変更の判定を行うときの条件として用いる際の帳尻を合わせるため
end
...
while ab_point[0].size > 0 || ab_point[1].size > 0
  ...
  # 上で n に代入した数値（距離）が、それまで代入していた値より 1 大きい場合、そのプレイヤーの、それまでの距離における操作回数が操作可能回数に達したことになり、プレイヤーを交代させる必要がある
  # 要は、その距離 n で行うべき操作が終了したことを示唆している
  # 距離 n は操作回数の範囲の指標となるものと捉えることが重要
